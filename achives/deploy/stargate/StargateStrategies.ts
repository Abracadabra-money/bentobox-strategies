...
const deployFunction: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  switch (chainId) {
    ...
    case ChainId.Avalanche:
      ...
      UsdtStargateLPStrategy = await wrappedDeploy<StargateLPStrategy>("AvalancheUsdtStargateLPStrategyV1", {
        from: deployer,
        args: [
      ...
      UsdcSwapper = await wrappedDeploy("AvalancheStargateUsdcSwapperV1", {
        from: deployer,
        args: [[Constants.avalanche.stargate.stg, Constants.avalanche.usdc]],
        log: true,
        deterministicDeployment: false,
        contract: "AvalancheStargateSwapperV1",
      });
      UsdtSwapper = await wrappedDeploy("AvalancheStargateUsdtSwapperV1", {
        from: deployer,
        args: [[Constants.avalanche.stargate.stg, Constants.avalanche.usdc, Constants.avalanche.usdt]],
        log: true,
        deterministicDeployment: false,
        contract: "AvalancheStargateSwapperV1",
      });
